...
def yolo_detect(img, yolo_model, out_layers):   # YOLO 모델로 물체를 검출
    ...
    test_img = cv.dnn.blobFromImage(img, 1.0/255.0, (416, 416), (0, 0, 0), swapRB=True, crop=False)
    ...
    for output in output3:
        for vec85 in output:
            ...
            if confidence>0.5:  # 신뢰도가 50% 이상인 경우만 취함
                centerx, centery = int(vec85[0]*height), int(vec85[1]*height)
